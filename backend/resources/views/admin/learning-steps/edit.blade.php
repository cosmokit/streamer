...
@section('title', 'Редактировать Шаг')
...
<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ route('admin.learning-steps.update', $learningStep) }}">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" 
                       value="{{ old('title', $learningStep->title) }}" required>
                @error('title')
            ...
            <div class="mb-3">
                ...
                <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                          rows="3" required>{{ old('description', $learningStep->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="url" name="external_url" class="form-control @error('external_url') is-invalid @enderror" 
                       value="{{ old('external_url', $learningStep->external_url) }}" required>
                @error('external_url')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="order" class="form-control @error('order') is-invalid @enderror" 
                       value="{{ old('order', $learningStep->order) }}" min="0" required>
                @error('order')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3 form-check">
                ...
                <input type="checkbox" name="is_active" value="1" class="form-check-input" 
                       id="is_active" {{ old('is_active', $learningStep->is_active) ? 'checked' : '' }}>
